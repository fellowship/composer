...
class ProgressiveResizing(Algorithm):
    ...
    Args:
        ...
        pin_sizes (int, optional): Make sizes to be a multiple of this number. Default: ``1``.
    ...
    def apply(self, event: Event, state: State, logger: Optional[Logger] = None) -> None:
        ...
        dur_elapsed = state.get_elapsed_duration().value
        if dur_elapsed >= self.delay_fraction:
            scale_frac_elapsed = min([
                (dur_elapsed - self.delay_fraction) / (1 - self.finetune_fraction - self.delay_fraction), 1
            ])
        else:
        ...
        # adjust scale factor so that we make width a multiple of pin_sizes
        width = input.shape[3]
        scaled_width_pinned = round(width * scale_factor / self.pin_sizes) * self.pin_sizes
